...
-export([has_removals/1]).

-include("services.hrl").

-record(kz_service_item, {item_plan = kz_json:new() :: kz_json:object()
                         ,category_name :: kz_term:api_binary()
...
log_item(_Services, Item) ->
    case billable_quantity(Item) > 0 of
        'false' -> Item;
        'true' ->
            lager:debug("~p billable ~s at ~p each with ~p discounts"
...
calculate_billable_quantity(_Services, Item) ->
    ...
    case minimum(Item) of
        Minimum when Minimum > BillableQuantity ->
...
%% @end
%%------------------------------------------------------------------------------
-spec has_removals(item()) -> boolean().
has_removals(Item) ->
    case changes(Item) of
        'undefined' -> 'false';
        Changes ->
